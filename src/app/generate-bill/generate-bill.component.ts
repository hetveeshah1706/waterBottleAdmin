...
export class GenerateBillComponent implements OnInit {
  ...
  displayedColumns1:string[]=['customer_name', 'pro_name', 'pro_price', 'qty', 'order_date', 'order_status'];
  ...
  ngOnInit() {
    this.order_id = this._activateRoutes.snapshot.params['order_id'];
    this._billser.getDataByID(this.order_id).subscribe(
      (data: any) => {
        console.log(data);
        ...
        this._billser.getorderbycustomerid(this.customer_id).subscribe(
          (data:any)=>{
            console.log(data)
            this.dataSource1.data=data
          }
